...
class MTTG(object):
    ...
    def process(self, outfit_id, data):
        ...
        for outfit_i in outfit_id:
            ...
            for idx, item_i in enumerate(data_dict):
                title_i[idx * self.text_dim:(idx + 1) * self.text_dim] = self.text_dict[item_i]
                item.append(self.imgArr_dict[item_i])
            title.append(title_i)
        return outfit, item, item_num, title
    ...
    def train(self):
        ...
        visual_code, visual_info, text_code, text_info, Pv, Pt = self.mttg(self.item, self.item_num, self.item_text)

        self.outfit_ = self.outfit
        self.Pv_ = Pv
        self.Pt_ = Pt
        ...
        self.L_Gv = tf.reduce_mean(tf.abs(self.outfit - Pv))
        ...
        for step in range(20001):
            ...
            outfit, item, item_num, title = self.process(batch_outfits, self.data)

            _, _, L_Gt, L_Gv = self.sess.run([self.Gv_optim, self.Gt_optim, self.L_Gt, self.L_Gv],
                                                    {self.outfit: outfit, self.item: item,
                                                     self.item_num: item_num, self.item_text: title,
    ...
    def sample_model(self, sample_path, step):
        ...
        for idxi, test_outfits in enumerate(batch_test_outfits):
            real_outfits, items, item_nums, title = self.process(test_outfits, self.data)
            Pv, Pt = self.sess.run([self.Pv_, self.Pt_], {self.item: items, self.item_num: item_nums, self.item_text: title,
    ...
    def textual_encoder(self, item_text, reuse=False):
        with tf.variable_scope('textual') as scope:
            ...
            hv4 = deconv2d(tf.nn.relu(ht3), [dim1, 16, 16, 64 * 8], name='g_ht4')
            hv5 = deconv2d(tf.nn.relu(hv4), [dim1, 32, 32, 64 * 4], name='g_ht5')
            hv6 = deconv2d(tf.nn.relu(hv5), [dim1, 64, 64, 64 * 2], name='g_ht6')
            hv7 = deconv2d(tf.nn.relu(hv6), [dim1, 128, 128, 64], name='g_ht7')

            info = [hv7, hv6, hv5, hv4, ht3, ht2, ht1, transd]
...
mttg_model = MTTG(sess=sess, checkpoint_path='./checkpoint/', sample_path='./sample/',
                        data_path = '/home/share/yaofubin/', batch_size=27)
